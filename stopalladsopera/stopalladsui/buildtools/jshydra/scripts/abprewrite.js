...
process_js = function(ast, filename, args)
{
  ...
  if (!headerPrinted)
  {
    ...
    // is being processed.
    _print('/*');
     _print(' * This file is part of StopAll Ads <http://www.stopallads.com/>,');
    _print(' * Copyright (C) 2016 Tweaking Technologies');
    _print(' *');
    _print(' * StopAll Ads is free software: you can redistribute it and/or modify');
    _print(' * it under the terms of the GNU General Public License version 3 as');
    _print(' * published by the Free Software Foundation.');
    _print(' *');
    _print(' * StopAll Ads is distributed in the hope that it will be useful,');
    ...
    _print(' * along with StopAll Ads.  If not, see <http://www.gnu.org/licenses/>.');
    _print(' */');
    _print();
    _print('//');
    _print();
