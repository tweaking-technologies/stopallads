...
var Backup =
{
  ...
  restoreAllData: function(/**nsIFile*/ file)
  {
    ...
    let match;
    if (lines.length < 2 || lines[0] != "# StopAll Ads preferences" || !(match = /version=(\d+)/.exec(lines[1])))
    {
    ...
    IO.readFromFile(file, {
      ...
      process: function(line)
      {
        if (!this.seenHeader)
        {
          ...
          let match = /\[StopAll(?:\s*Ads\s*([\d\.]+)?)?\]/i.exec(line);
          if (match)
          {
  ...
  backupCustomFilters: function(/**nsIFile*/ file)
  {
    ...
    for (let i = 0; i < subscriptions.length; i++)
    {
      ...
      for (let j = 0; j < subscription.filters.length; j++)
      {
		...
		//StopAll Ads changed min version comparison check as we don't need
		minVersion = "1.0";
    ...
    list.unshift("[StopAll Ads " + minVersion + "]");
