...
var Backup =
{
  ...
  restoreAllData: function(/**nsIFile*/ file)
  {
    ...
    let match;
    if (lines.length < 2 || lines[0] != "# Stop All Ads preferences" || !(match = /version=(\d+)/.exec(lines[1])))
    {
  ...
  backupCustomFilters: function(/**nsIFile*/ file)
  {
    ...
    for (let i = 0; i < subscriptions.length; i++)
    {
      ...
      for (let j = 0; j < subscription.filters.length; j++)
      {
		...
		//Stop all ads changed min version comparison check as we don't need
		minVersion = "1.0";
    ...
    list.unshift("[Stop All Ads " + minVersion + "]");
