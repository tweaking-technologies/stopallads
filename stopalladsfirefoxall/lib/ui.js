...
let optionsObserver =
{
  ...
  initOptionsDoc: function(/**Document*/ doc)
  {
    ...
    addCommandHandler("stopallads-filters", UI.openFiltersDialog.bind(UI));

    let {Sync} = require("sync");
    let syncEngine = Sync.getEngine();
    hideElement("stopallads-sync", !syncEngine);
    ...
    hideElement("stopallads-showintoolbar", !hasToolbar);
    hideElement("stopallads-showinstatusbar", !hasStatusBar);
    ...
    function initCheckboxes()
    {
      ...
      setChecked("stopallads-savestats", Prefs.savestats);
      addCommandHandler("stopallads-savestats", function()
      {
        UI.toggleSaveStats(doc.defaultView);
        this.value = Prefs.savestats;
      });

      hideElement("stopallads-shownotifications", !Prefs.notifications_showui);
      setChecked("stopallads-shownotifications", Prefs.notifications_ignoredcategories.indexOf("*") == -1);
      addCommandHandler("stopallads-shownotifications", function()
      {
      ...
      setChecked("stopallads-acceptableAds", hasAcceptableAds);
      addCommandHandler("stopallads-acceptableAds", function()
      {
        this.value = UI.toggleAcceptableAds();
      });

      setChecked("stopallads-sync", syncEngine && syncEngine.enabled);
      addCommandHandler("stopallads-sync", function()
      {
        this.value = UI.toggleSync();
      });

      setChecked("stopallads-showintoolbar", UI.isToolbarIconVisible());
      addCommandHandler("stopallads-showintoolbar", function()
      {
        UI.toggleToolbarIcon();
        this.value = UI.isToolbarIconVisible();
      });

      let list = doc.getElementById("stopallads-subscription-list");
      if (list)
      {
        // Load subscriptions data
        let request = new XMLHttpRequest();
        request.mozBackgroundRequest = true;
        request.open("GET", "chrome://stopallads/content/ui/subscriptions.xml");
...
let UI = exports.UI =
{
  ...
  init: function()
  {
    ...
    let request = new XMLHttpRequest();
    request.mozBackgroundRequest = true;
    request.open("GET", "chrome://stopallads/content/ui/overlay.xul");
  ...
  processOverlay: function(/**Element*/ root)
  {
    ...
    let specialElements = {"saa-status-popup": true, "saa-status": true, "saa-toolbarbutton": true, "saa-menuitem": true, "saa-bottombar-container": true};
    ...
    function fixId(element, newId)
    {
      if (element.hasAttribute("id"))
        element.setAttribute("id", element.getAttribute("id").replace("saa-status", newId));
    ...
    if ("saa-status-popup" in this.overlay)
    {
      let menuSource = this.overlay["saa-status-popup"];
      delete this.overlay["saa-status-popup"];

      if (this.overlay.all.length)
        this.overlay.all[0].appendChild(menuSource);
      if ("saa-toolbarbutton" in this.overlay)
        this.overlay["saa-toolbarbutton"].appendChild(fixId(menuSource.cloneNode(true), "saa-toolbar"));
      if ("saa-menuitem" in this.overlay)
        this.overlay["saa-menuitem"].appendChild(fixId(menuSource.cloneNode(true), "saa-menuitem"));
    }
  },

  /**
   * Gets called once the initialization is finished and Stop All Ads elements
  ...
  initDone: function()
  {
    ...
    Notification.addShowListener(notification =>
    {
      ...
      let button = window.document.getElementById("saa-toolbarbutton")
          || window.document.getElementById("saa-status");
      if (!button)
        return;
  ...
  addToolbarButton: function()
  {
    ...
    if ("saa-toolbarbutton" in this.overlay && defaultToolbarPosition)
    {
      try
      {
      ...
      CustomizableUI.createWidget({
        id: "saa-toolbarbutton",
        type: "custom",
        positionAttribute: "saa-iconposition",        // For emulation only
        ...
        {
          let node = document.importNode(this.overlay["saa-toolbarbutton"], true);
      ...
      onShutdown.add(CustomizableUI.destroyWidget.bind(CustomizableUI, "saa-toolbarbutton"));
    }
  },
  ...
  /**
   * Initializes Stop All Ads UI in a window.
   */
  applyToWindow: function(/**Window*/ window, /**Boolean*/ noDelay)
  {
    ...
    if (window.document.documentElement.id == "CustomizeToolbarWindow" || isKnownWindow(window))
    {
      ...
      let style = window.document.createProcessingInstruction("xml-stylesheet", 'class="stopallads-node" href="chrome://stopallads/skin/overlay.css" type="text/css"');
    ...
    if ("saa-menuitem" in this.overlay)
    {
      ...
      if (parent)
        parent.insertBefore(this.overlay["saa-menuitem"].cloneNode(true), before);
    }
    ...
    {
      this.updateIconState(window, window.document.getElementById("saa-status"));
      this.updateIconState(window, window.document.getElementById("saa-toolbarbutton"));

      Notification.showNext(this.getCurrentLocation(window).spec);
    }.bind(this));

    let notificationPanel = window.document.getElementById("saa-notification");
    notificationPanel.addEventListener("command", function(event)
    {
      switch (event.target.id)
      {
        case "saa-notification-close":
          notificationPanel.classList.add("saa-closing");
          break;
        case "saa-notification-optout":
          Notification.toggleIgnoreCategory("*", true);
          /* FALL THROUGH */
        case "saa-notification-hide":
  ...
  /**
   * Removes Stop All Ads UI from a window.
   */
  removeFromWindow: function(/**Window*/ window)
  {
    ...
    if (window.document.documentElement.id == "CustomizeToolbarWindow" || isKnownWindow(window))
    {
      ...
      for (let child = window.document.firstChild; child; child = child.nextSibling)
        if (child.nodeType == child.PROCESSING_INSTRUCTION_NODE && child.data.indexOf("stopallads-node") >= 0)
  ...
  /**
   * Iterator for application windows that Stop All Ads should apply to.
   * @type Iterator
   */
   ...
   * send the UI language to stopallads.org so that the correct language
  ...
  blockItem: function(/**Window*/ window, /**string*/ nodesID, /**RequestEntry*/ item)
  {
    ...
    window.openDialog("chrome://stopallads/content/ui/composer.xul", "_blank",
  ...
  {
    let existing = Services.wm.getMostRecentWindow("saa:filters");
    if (existing)
    {
      try
      {
    ...
    {
      Services.ww.openWindow(null, "chrome://stopallads/content/ui/filters.xul", "_blank", "chrome,centerscreen,resizable,dialog=no", {wrappedJSObject: filter});
    }
  },
  ...
  {
    let wnd = Services.wm.getMostRecentWindow("saa:sendReport");
    if (wnd)
      wnd.focus();
    else
    {
      ...
      if (uri)
      {
        ...
        window.openDialog("chrome://stopallads/content/ui/sendReport.xul", "_blank", "chrome,centerscreen,resizable=no", browser.outerWindowID, uri, browser);
      }
    }
  },
  ...
  addSubscription: function(/**Window*/ window, /**String*/ prevVersion)
  {
    ...
    let privacySubscriptions = {
      "http://stopallads.s3.amazonaws.com/subscriptions/easyprivacy_easylist.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/easyprivacy.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/fanboy-tracking.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/fanboy-adblocklist-stats.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/fanboy-adblocklist-stats.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/fanboy-adblocklist-stats.txt": true,
      "http://stopallads.s3.amazonaws.com/subscriptions/Adversity-Tracking.txt": true
    };
    ...
    function notifyUser()
    {
      ...
      if (addTab)
      {
        addTab(window, "chrome://stopallads/content/ui/after-install.html");
      }
      else
      {
        let dialogSource = '\
          ...
          <dialog xmlns="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul" onload="document.title=content.document.title" buttons="accept" width="500" height="600">\
            <iframe type="content-primary" flex="1" src="chrome://stopallads/content/ui/after-install.html"/>\
          </dialog>';
   ...
   * as well as links to subscribe.stopallads.org.
   */
  ...
  openSubscriptionDialog: function(/**Window*/ window, /**String*/ url, /**String*/ title, /**String*/ mainURL, /**String*/ mainTitle)
  {
    ...
    window.openDialog("chrome://stopallads/content/ui/subscriptionSelection.xul", "_blank",
  ...
  toggleToolbarIcon: function()
  {
    ...
    if (this.isToolbarIconVisible())
      CustomizableUI.removeWidgetFromArea("saa-toolbarbutton");
    else
    {
      let {defaultToolbarPosition} = require("appSupport");
      CustomizableUI.addWidgetToArea("saa-toolbarbutton", defaultToolbarPosition.parent);
    }
  },

  /**
   * Updates Stop All Ads icon state for all windows.
   */
  ...
  /**
   * Updates Stop All Ads icon state for a single application window.
   */
  updateIconState: function(/**Window*/ window, /**Element*/ icon)
  {
    ...
    icon.setAttribute("saastate", state);
  },
  ...
  updateStatusbarIcon: function(/**Window*/ window)
  {
    ...
    let icon = window.document.getElementById("saa-status");
    if (Prefs.showinstatusbar && !icon)
    {
      ...
      parent.insertBefore(this.overlay["saa-status"].cloneNode(true), before);

      icon = window.document.getElementById("saa-status");
  ...
  /**
   * Toggles the pref for the Stop All Ads sync engine.
  ...
  isBottombarOpen: function(/**Window*/ window) /**Boolean*/
  {
    ...
    return !!window.document.getElementById("saa-bottombar");
  },

  /**
   * Called when some pop-up in the application window shows up, initializes
   * pop-ups related to Stop All Ads.
   */
  onPopupShowing: function(/**Event*/ event)
  {
    ...
    else if (popup.id == "saa-tooltip")
      this.fillIconTooltip(event, popup.ownerDocument.defaultView);
    else
    {
      let match = /^(saa-(?:toolbar|status|menuitem)-)popup$/.exec(popup.id);
      if (match)
  ...
  fillIconTooltip: function(/**Event*/ event, /**Window*/ window)
  {
    ...
    for (let id of ["saa-toolbar-popup", "saa-status-popup"])
    {
      let element = E(id);
    ...
    let type = (node.id == "saa-toolbarbutton" ? "toolbar" : "statusbar");
    let action = parseInt(Prefs["default" + type + "action"]);
    if (isNaN(action))
      action = -1;

    let actionDescr = E("saa-tooltip-action");
    ...
    let statusDescr = E("saa-tooltip-status");
    let state = node.getAttribute("saastate");
    ...
    E("saa-tooltip-blocked-label").hidden = true;
    E("saa-tooltip-blocked").hidden = true;
    E("saa-tooltip-filters-label").hidden = true;
    E("saa-tooltip-filters").hidden = true;
    E("saa-tooltip-more-filters").hidden = true;
    ...
    if (state == "active")
    {
      ...
      {
        E("saa-tooltip-blocked-label").hidden = false;
        E("saa-tooltip-blocked").hidden = false;
        ...
        E("saa-tooltip-blocked").setAttribute("value", blockedStr);
        ...
        {
          let filtersContainer = E("saa-tooltip-filters");
        ...
        E("saa-tooltip-filters-label").hidden = (activeFilters.length == 0);
        E("saa-tooltip-filters").hidden = (activeFilters.length == 0);
        E("saa-tooltip-more-filters").hidden = (activeFilters.length <= 3);
      });
    }
  },
  ...
  fillIconMenu: function(/**Event*/ event, /**Window*/ window, /**String*/ prefix)
  {
    ...
    setDisabled("saa-command-sendReport", !location || !Policy.isBlockableScheme(location) || location.scheme == "mailto");
    ...
    let hasToolbar = defaultToolbarPosition;
    let hasStatusBar = statusbarPosition;
    hideElement(prefix + "showintoolbar", !hasToolbar || prefix == "saa-toolbar-");
    hideElement(prefix + "showinstatusbar", !hasStatusBar);
    hideElement(prefix + "shownotifications", !Prefs.notifications_showui);
    hideElement(prefix + "iconSettingsSeparator", (prefix == "saa-toolbar-" || !hasToolbar) && !hasStatusBar);
    ...
    let defAction = (!window.document.popupNode || window.document.popupNode.id == "saa-toolbarbutton" ?
    ...
    {
      let item = items[i];
      let match = /^saa-key-/.exec(item.getAttribute("key"));
      if (!match)
        continue;
  ...
  /**
   * Adds Stop All Ads menu items to the content area context menu when it shows
   * up.
   */
  fillContentContextMenu: function(/**Element*/ popup)
  {
    ...
    if (!data)
    {
      ...
      Services.obs.notifyObservers(data, "stopallads:content-contextmenu", null);
    }

    if (typeof data.addonInfo != "object" || typeof data.addonInfo.stopallads != "object")
      return;

    let items = data.addonInfo.stopallads;
    let clicked = null;
    let menuItems = [];
    ...
    for (let [id, nodeData] of items)
    {
      ...
      item.setAttribute("class", "saa-contextmenuitem");
  ...
  onKeyPress: function(/**Event*/ event)
  {
    ...
    for (let key in this.hotkeys)
    {
      ...
      {
        event.preventDefault();
        let command = event.currentTarget.document.getElementById("saa-command-" + key);
        if (command)
          command.doCommand();
      }
    }
  ...
  isToolbarIconVisible: function() /**Boolean*/
  {
    if (!CustomizableUI)
      return false;
    let placement = CustomizableUI.getPlacementOfWidget("saa-toolbarbutton");
    return !!placement;
  },
  ...
  toggleBottombar: function(/**Window*/ window)
  {
    ...
    else
    {
      ...
      let mustDetach = !addBottomBar || !removeBottomBar || !("saa-bottombar-container" in this.overlay);
      let detach = mustDetach || Prefs.detachsidebar;
      if (!detach && window.document.getElementById("saa-bottombar"))
      {
      ...
      {
        addBottomBar(window, this.overlay["saa-bottombar-container"]);
        let element = window.document.getElementById("saa-bottombar");
        if (element)
        {
          element.setAttribute("width", Prefs.blockableItemsSize.width);
          element.setAttribute("height", Prefs.blockableItemsSize.height);

          let splitter = window.document.getElementById("saa-bottombar-splitter");
          if (splitter)
          {
        ...
        detachedBottombar = window.openDialog("chrome://stopallads/content/ui/sidebarDetached.xul", "_blank", "chrome,resizable,dependent,dialog=no", mustDetach);
    }
  },
  ...
  hideContributeButton: function(/**Window*/ window)
  {
    ...
    for (let id of ["saa-status-contributebutton", "saa-toolbar-contributebutton", "saa-menuitem-contributebutton"])
    {
  ...
  {
    let panel = window.document.getElementById("saa-notification");
    ...
    let titleElement = window.document.getElementById("saa-notification-title");
    titleElement.textContent = texts.title;
    let messageElement = window.document.getElementById("saa-notification-message");
    ...
    if (notification.type === "question")
    {
      ...
      window.document.getElementById("saa-notification-yes").onclick = buttonHandler.bind(null, true);
      window.document.getElementById("saa-notification-no").onclick = buttonHandler.bind(null, false);
    }
    else
      Notification.markAsShown(notification.id);

    panel.setAttribute("class", "saa-" + notification.type);
 ...
 */
let eventHandlers = [
  ["saa-command-sendReport", "command", UI.openReportDialog.bind(UI)],
  ["saa-command-filters", "command", UI.openFiltersDialog.bind(UI)],
  ["saa-command-sidebar", "command", UI.toggleBottombar.bind(UI)],
  ["saa-command-togglesitewhitelist", "command", function() { UI.toggleFilter(siteWhitelist); }],
  ["saa-command-togglepagewhitelist", "command", function() { UI.toggleFilter(pageWhitelist); }],
  ["saa-command-toggleobjtabs", "command", UI.togglePref.bind(UI, "frameobjects")],
  ["saa-command-togglesavestats", "command", UI.toggleSaveStats.bind(UI)],
  ["saa-command-togglesync", "command", UI.toggleSync.bind(UI)],
  ["saa-command-toggleshowintoolbar", "command", UI.toggleToolbarIcon.bind(UI)],
  ["saa-command-toggleshowinstatusbar", "command", UI.togglePref.bind(UI, "showinstatusbar")],
  ["saa-command-enable", "command", UI.togglePref.bind(UI, "enabled")],
  /*["saa-command-contribute", "command", UI.openContributePage.bind(UI)],
  ["saa-command-contribute-hide", "command", UI.hideContributeButton.bind(UI)],*/
  ["saa-command-toggleshownotifications", "command", Notification.toggleIgnoreCategory.bind(Notification, "*", null)]
];
