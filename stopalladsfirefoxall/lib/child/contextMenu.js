...
let ContextMenuObserver =
{
  observe: function(subject, topic, data)
  {
    ...
    if (subject.addonInfo)
      subject.addonInfo.stopallads = getContextInfo(subject.event);
  },
  QueryInterface: XPCOMUtils.generateQI([Ci.nsISupportsWeakReference, Ci.nsIObserver])
};

Services.obs.addObserver(ContextMenuObserver, "content-contextmenu", true);
Services.obs.addObserver(ContextMenuObserver, "stopallads:content-contextmenu", true);
onShutdown.add(() => {
  Services.obs.removeObserver(ContextMenuObserver, "content-contextmenu");
  Services.obs.removeObserver(ContextMenuObserver, "stopallads:content-contextmenu");
});
